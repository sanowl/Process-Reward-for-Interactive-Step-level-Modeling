...
Runs the full PRISM pipeline for a chosen environment: collect trajectories,
Monte-Carlo label them, train a process reward model, then train and compare a
terminal-reward baseline policy against a PRISM policy that adds potential-based
PRM shaping. Writes trajectories, labels, the PRM, learning curves (CSV + SVG),
and a summary to the output directory. Invoke via ``python train.py`` or the
``prism`` console script.
"""
